...
NSString *kPrivateDragUTI = @"moe.cocoaneko.cocoadraganddrop";
...
- (void)mouseDown:(NSEvent*)event
{
    if (self.allowDrag) {
        ...
        [self dragPromisedFilesOfTypes:[NSArray arrayWithObject:NSPasteboardTypePDF] fromRect:imageLocation source:self slideBack:YES event:event];
    }
}
...
- (NSArray *)namesOfPromisedFilesDroppedAtDestination:(NSURL *)dropDestination
{
    ...
    NSString *filename = self.path.lastPathComponent ? [NSString stringWithFormat:@"%@-2x.png", self.path.lastPathComponent] : @"waifu2x-output.png";
